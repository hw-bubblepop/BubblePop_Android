            ...
            NetworkHelper.networkInstance.createChat("private", "asdf").enqueue(object : Callback<ChatRoom> {
                ...
                override fun onResponse(call: Call<ChatRoom>?, response: Response<ChatRoom>?) {
                    response?.let {
                        ...
                        NetworkHelper.networkInstance.addUserToChat(it.body()?._id!!, user._id).enqueue(object: Callback<ChatRoom>{
                            override fun onFailure(call: Call<ChatRoom>?, t: Throwable?) {
                            }
                        ...
                        NetworkHelper.networkInstance.addUserToChat(it.body()?._id!!, CredentialsManager.instance.activeUser.second._id).enqueue(object: Callback<ChatRoom>{
                            override fun onFailure(call: Call<ChatRoom>?, t: Throwable?) {
                            }
